...
public class TripApiController implements TripApi {
    ...
    @Override
    public ResponseEntity<Trip> generateTrip(@NotNull @Valid Long userId, @NotNull @Valid Long cityId, @NotNull @Valid ArrayList<Stop> stops) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                travlynService.generateTrip(userId, cityId,stops);
